...
while supervisor.step(TIME_STEP) != -1:
    ...
    # get geometry of specific death zone
    ...
    if robot_below_zone(values1, deathzone_xyz1):
        os.system('echo \\nRobo Sumo, ' + cont1str + ', ' + cont2str + ', 2 >> ..\\..\\..\\UI\\public\\gamescores.txt')
        supervisor.setLabel(1, "robot 2 wins", 0.2, 0.2, 0.1, 000, 0, "Arial")
        supervisor.simulationQuit(1)
    if robot_below_zone(values2, deathzone_xyz1):
        os.system('echo \\nRobo Sumo, ' + cont1str + ', ' + cont2str + ', 1 >> ..\\..\\..\\UI\\public\\gamescores.txt')
        supervisor.setLabel(1, "robot 1 wins", 0.2, 0.2, 0.1, 000, 0, "Arial")
        supervisor.simulationQuit(1)
    if Robot.getTime(supervisor) > 60:
        os.system('echo \\nRobo Sumo, ' + cont1str + ', ' + cont2str + ', 4 >> ..\\..\\..\\UI\\public\\gamescores.txt')
        supervisor.setLabel(1, "robot 1 wins", 0.2, 0.2, 0.1, 000, 0, "Arial")
        supervisor.simulationQuit(1)
    ...
    if keypressed == 32:
        if (1 == supervisor.simulationGetMode()):
            os.system('echo \\nRace, Bobsrace, ExampleCont, 2 >> ..\\..\\..\\UI\\public\\gamescores.txt')
            supervisor.simulationSetMode(Supervisor.SIMULATION_MODE_PAUSE)
        else:
