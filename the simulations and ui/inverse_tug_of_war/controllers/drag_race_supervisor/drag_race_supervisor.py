from controller import Supervisor, Keyboard, AnsiCodes, Robot
import sys, os
...
while supervisor.step(TIME_STEP) != -1:
    ...
    if robot_in_zone(values1, deathzone_xyz1, 0.4)and robot_in_zone(values2, deathzone_xyz2, 0.4):
        result = 3
    elif robot_in_zone(values1, deathzone_xyz1, 0.4):
        result = 1 
    elif robot_in_zone(values2, deathzone_xyz2, 0.4):
        result = 2
	
    if result ==1:
        os.system('echo \\nDrag Race, ' + cont1str + ', ' + cont2str + ', 1 >> ..\\..\\..\\UI\\public\\gamescores.txt')
        supervisor.setLabel(1, "robot 1 wins", 0.2, 0.2, 0.1, 000, 0, "Arial")
        supervisor.simulationQuit(1)
    elif result==2:
        os.system('echo \\nDrag Race, ' + cont1str + ', ' + cont2str + ', 2 >> ..\\..\\..\\UI\\public\\gamescores.txt')
        supervisor.setLabel(1, "robot 2 wins", 0.2, 0.2, 0.1, 000, 0, "Arial")
        supervisor.simulationQuit(1)
    elif result==3:
        os.system('echo \\nDrag Race, ' + cont1str + ', ' + cont2str + ', 3 >> ..\\..\\..\\UI\\public\\gamescores.txt')
        supervisor.setLabel(1, "draw", 0.2, 0.2, 0.1, 000, 0, "Arial")
        supervisor.simulationQuit(1)
		
    if Robot.getTime(supervisor) > 60:
        os.system('echo \\nDrag Race, ' + cont1str + ', ' + cont2str + ', 4 >> ..\\..\\..\\UI\\public\\gamescores.txt')
